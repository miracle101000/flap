...
mod tests {
    ...
    fn dir_name_from_local_path() {
        ...
        assert_eq!(spec_to_dir_name("secure_petstore.yml"), "secure_petstore");
        assert_eq!(spec_to_dir_name("api.json"), "api");
    }

    #[test]
    fn dir_name_from_url() {
        assert_eq!(
            spec_to_dir_name("https://example.com/api/v3/openapi.yaml"),
            "openapi"
        );
        assert_eq!(
            spec_to_dir_name("https://example.com/api/v3/openapi.yaml?version=2"),
            "openapi"
        );
    }

    #[test]
    fn dir_name_fallback() {
        assert_eq!(spec_to_dir_name(""), "spec");
    }

    #[test]
    fn parse_args_single_spec() {
        let args = to_args(&["--out", "/tmp/out", "spec.yaml"]);
        let (dir, specs) = parse_args(&args).unwrap();
        assert_eq!(dir, PathBuf::from("/tmp/out"));
        assert_eq!(specs, vec!["spec.yaml"]);
    }

    #[test]
    fn parse_args_multiple_specs() {
        let args = to_args(&[
            "--out",
            "/tmp/out",
            "a.yaml",
    ...
    #[test]
    fn parse_args_missing_out_is_error() {
        let args = to_args(&["spec.yaml"]);
